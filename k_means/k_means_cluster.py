...
for i in data_dict:
    if data_dict[i]["exercised_stock_options"] != "NaN":
        l.append(int(data_dict[i]["exercised_stock_options"]))
print max(l), min(l)
...
for i in range(len(finance_features)):
    s.append(finance_features[i][0])
    st.append(finance_features[i][1])

print float(200000-min(s))/(max(s)-min(s))
print float(1000000-min(st))/(max(st)-min(st))
